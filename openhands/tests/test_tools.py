...
import asyncio
import os
from openhands import EmbeddedAgent, AgentConfig, tool_registry
...
async def test_file_tools():
    ...
    test_content = "Hello from OpenHands test!"
...
async def test_terminal_tools():
    ...
    result = await registry.execute_tool(
        "terminal_run",
        {"command": "echo 'OpenHands terminal test'"}
    )
    print(f"terminal_run: {result.content[:100]}")

    return "OpenHands terminal test" in result.content
...
async def main():
    print("OpenHands Tool Call Test Suite")
    print("=" * 50)
